...
until RunMode = 0 {
    ...
    print ImpactData()["tillImpact"] at (1, 0).
    if addons:tr:hasimpact {
        print addons:tr:timetillimpact at (1, 1).
    }
    wait 0.
}
...
function steer {
    if RunMode = 1 {
        ...
        if ship:verticalSpeed < 0 and ship:altitude < 60000 {
            return LandPid(-1.5, 0.5, ship:velocity:surface, LatLngError(CorrPoint(LZPo, 0.2))[0]).
        }
        ...
        return LandPid(line(0, 1, 100, 35, LatLngError(CorrPoint(LZPo, 0.1))[1], 1, 35), 2, ship:velocity:surface, LatLngError(CorrPoint(LZPo, 0.05))[0]).
    }
    else if RunMode = 5 {
        if TrueAlt() < 30 {
            return lookDirUp(ship:up:vector, facing:topvector).
        }
        return LandPid(line(0, -0.1, 1000, -10, TrueAlt(), -10, -0.1), 3, ship:velocity:surface, LatLngError(CorrPoint(LZPo, 0))[0]).
    }
