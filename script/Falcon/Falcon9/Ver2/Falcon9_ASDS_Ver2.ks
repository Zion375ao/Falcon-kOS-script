...
function steer {
    if runmode = 1 {
        ...
        if ship:verticalSpeed < 0 and ship:altitude < 55000 {
            return steerpid(-1.5, 0.5, ship:velocity:surface, errorvector(refpoint(0.2, LZ))).
        }
        else {
        ...
        return steerpid(line(0, 7, 500, 12, errordiff(refpoint(0.08, LZ)), true, 1), 1.8, ship:velocity:surface, errorvector(refpoint(0.08, LZ))).
    }
    else if runmode = 5 {
        if (errordiff(refpoint(0.1, LZ)) > 3 or ship:groundspeed > 1) and truealt() > 30 {
            return steerpid(-2, 2.5, ship:velocity:surface, errorvector(refpoint(0.1, LZ))).
        }
        else {
