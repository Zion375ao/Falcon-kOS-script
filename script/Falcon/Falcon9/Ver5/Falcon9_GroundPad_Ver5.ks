...
until runmode = 0 {
    ...
    //BoostBack Burn
        ...
        if LatLngError(CorrPoint(LZPo, -0.4))[3] < 0 {
            brakes on.
...
function steer {
    if runmode = 1 {
        ...
        return heading(-LZPo:heading, bbAngle, 0) - BoostbackPid(CorrPoint(LZPo, -0.4)).
    }
        ...
        return LandPid(line(0, -0.1, 1000, -10, TrueAlt(), -10, -0.1), 1.5, ship:velocity:surface, LatLngError(CorrPoint(LZPo, 0.2))[0]).
    }
